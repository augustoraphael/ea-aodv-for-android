...
import adhoc.aodv.ea.BatteryInformation;
...
public class Receiver implements Runnable {
	...
	private void helloMessageReceived(HelloPacket hello) {
		String energy_info = hello.getEnergyInfo();
		BatteryInformation.updateNEDB(energy_info);
		
		try {
			routeTableManager.setValid(hello.getSourceAddress(), hello.getSourceSeqNr());
		} catch (NoSuchRouteException e) {
			float routeCost = 100.0F/(float)EnergyAware.getBatteryLevel(hello.getSourceAddress());
	...
	private void routeReplyReceived(RREP rrep, int senderNodeAddress) {
		String energy_info = rrep.getEnergyInfo();
		BatteryInformation.updateNEDB(energy_info);
		
		float MMBCR = BatteryInformation.selectMMBCR(energy_info);
		...
		float routeCost = 100.0F/(float)EnergyAware.getBatteryLevel(senderNodeAddress);
		if(routeTableManager.createForwardRouteEntry(	senderNodeAddress,
													senderNodeAddress,
													Constants.UNKNOWN_SEQUENCE_NUMBER, 1, true, routeCost)){
		...
		if (rrep.getSourceAddress() != nodeAddress) {
			...
			BatteryInformation bi = new BatteryInformation(EnergyAware.getOwnBatteryLevel(), nodeAddress, System.currentTimeMillis()/1000);
			String bi_string = bi.toString(); 
			rrep.setEnergyInfo(rrep.getEnergyInfo()+"-"+bi_string);
		...
		try {
			...
			routeCost = 100.0F/(float)EnergyAware.getBatteryLevel(senderNodeAddress);
			routeTableManager.createForwardRouteEntry(	rrep.getDestinationAddress(), 
														senderNodeAddress, 
														rrep.getDestinationSequenceNumber(),
														rrep.getHopCount(),
														precursorNode, true, routeCost);
	...
	private void routeRequestReceived(RREQ rreq, int senderNodeAddress) {
		String energy_info = rreq.getEnergyInfo();
		Log.d("DEBUG", "Energy info recebido: "+energy_info);
		BatteryInformation.updateNEDB(energy_info);
		float MMBCR = BatteryInformation.selectMMBCR(energy_info);
		...
		try {
			...
			routeTableManager.createForwardRouteEntry(	rreq.getSourceAddress(),
														senderNodeAddress,
														rreq.getSourceSequenceNumber(),
														rreq.getHopCount(), true, routeCost);
		} catch (RouteNotValidException e) {
			try {
		...
		try {
			...
			if (rrep == null) {
				//Check if the node can join any route. If not, drop packet RREQ
				if (EnergyAware.getOwnBatteryLevel() > Constants.PRIMEIRO_LIMIAR) {
					BatteryInformation bi = new BatteryInformation(EnergyAware.getOwnBatteryLevel(), nodeAddress, System.currentTimeMillis()/1000);
					String bi_string = bi.toString(); 
					rreq.setEnergyInfo(rreq.getEnergyInfo()+"-"+bi_string);
	...
	private static boolean isIncomingSeqNrBetter(int incomingSeqNum, int currentSeqNum) {
		return isIncomingRouteInfoBetter(incomingSeqNum, currentSeqNum, 0, 1, 0, 1);
	}
	 ...
	 */
	protected static boolean isIncomingRouteInfoBetter(int incomingSeqNum, int currentSeqNum, int incomingHopCount, int currentHopCount, float incomingRouteCost, float currentRouteCost) {
		if (Math.abs(incomingSeqNum - currentSeqNum) > Constants.SEQUENCE_NUMBER_INTERVAL) {
			...
			if ((incomingSeqNum % Constants.SEQUENCE_NUMBER_INTERVAL) >= (currentSeqNum % Constants.SEQUENCE_NUMBER_INTERVAL)) {
				if ((incomingSeqNum % Constants.SEQUENCE_NUMBER_INTERVAL) == (currentSeqNum % Constants.SEQUENCE_NUMBER_INTERVAL)
						&& incomingRouteCost > currentRouteCost) {
					return false;
				}
				return true;
			} else {
			...
			if (incomingSeqNum >= currentSeqNum) {
				if (incomingSeqNum == currentSeqNum && incomingRouteCost > currentRouteCost) {
					return false;
				}
				return true;
			} else {
