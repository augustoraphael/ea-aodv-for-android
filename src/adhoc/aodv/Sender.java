...
public class Sender implements Runnable{
    ...
    private class NeighbourBroadcaster extends Thread {
    	...
    	public void run(){
    		while (true) {
    			if (EnergyAware.getOwnBatteryLevel() <= Constants.SEGUNDO_LIMIAR) {
				...
				while(keepBroadcasting){
					...
					if (EnergyAware.getOwnBatteryLevel() <= Constants.PRIMEIRO_LIMIAR) {
						//Para de enviar mensagens HELLO
						keepBroadcasting = false;
						
						//Foça o envio de RERR
						routeTableManager.sendRouteErrors();						
					...
					try {
	    				...
	    				queueHelloPacket(new HelloPacket(nodeAddress,parent.getCurrentSequenceNumber(), bi_string));
		    		} catch (InterruptedException e) {
