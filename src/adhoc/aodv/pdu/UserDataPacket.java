...
public class UserDataPacket implements Packet {
	...
	public UserDataPacket(int packetIdentifier, int destinationAddress,
			byte[] data, int sourceAddress) {
	...
	public void parseBytes(byte[] rawPdu) throws BadPduFormatException {
		...
		if (s.length != 4) {
			throw new BadPduFormatException(
					" UserDataPacket : could not split "
		...
		try {
			...
			if (pduType != Constants.USER_DATA_PACKET_PDU) {
				throw new BadPduFormatException(
						" UserDataPacket : pdu type did not match . "
								+ " Was expecting : "
			...
			throw new BadPduFormatException(
					" UserDataPacket : falied in parsing "
