...
public class RERR extends AodvPDU {
	 ...
	 */
	public RERR(int unreachableNodeAddress, int unreachableNodeSequenceNumber,
			ArrayList<Integer> destinationAddresses) {
	 ...
	 *            the node which hopefully will receive this PDU packet
	 */
	public RERR(int unreachableNodeAddress, int unreachableNodeSequenceNumber,
	...
	public String toString() {
		return Byte.toString(pduType) + ";" + unreachableNodeAddress + ";"
		...
		try {
			...
			if (pduType != Constants.RERR_PDU) {
				throw new BadPduFormatException(
						"RERR: pdu type did not match. " + "Was expecting: "
								+ Constants.RERR_PDU + "but parsed: " + pduType);
			...
			throw new BadPduFormatException(
					"RERR: falied in parsing arguments to the desired types ");
		}
	}
}
